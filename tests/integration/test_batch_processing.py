...
class TestBatchProcessingWithMalformedData:
    ...
    def test_batch_processing_10_emails_1_malformed_9_succeed(self):
        ...
        assert len(emails) == 9, (
            f"Expected 9 valid emails (1 skipped), got {len(emails)}"
        )
    ...
    def test_batch_processing_with_encoding_issues(self):
        ...
        assert len(emails) == 3, (
            f"Expected 3 emails (encoding handled), got {len(emails)}"
        )
        ...
        msg_002 = next(
            e for e in emails if e.metadata.message_id == "<msg_002@gmail.com>"
        )
        assert len(msg_002.body) > 0, (
            "Email body should be decoded (even with fallback)"
        )
    ...
    def test_batch_processing_with_no_date(self):
        ...
        mock_service.users().messages().list().execute.return_value = {
            "messages": [{"id": "msg_001"}, {"id": "msg_002_no_date"}]
        }
        ...
        assert len(emails) == 2, (
            f"Expected 2 emails (missing date handled), got {len(emails)}"
        )
