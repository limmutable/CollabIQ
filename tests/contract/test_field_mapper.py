...
class TestFieldMapperContract:
    ...
    def test_map_to_notion_properties_method_signature(
        ...
        # Verify contract
        assert isinstance(result, dict), "map_to_notion_properties must return dict"
        assert len(result) > 0, (
    ...
    def test_format_rich_text_with_korean_text(self, field_mapper):
        ...
        assert isinstance(result["rich_text"], list), "rich_text value must be a list"
        assert len(result["rich_text"]) > 0, "rich_text list must not be empty"
        assert "text" in result["rich_text"][0], "rich_text item must have 'text' key"
    ...
    def test_format_select_with_collaboration_type(self, field_mapper):
        ...
        assert isinstance(result["select"], dict), "select value must be a dict"
        assert "name" in result["select"], "select dict must have 'name' key"
    ...
    def test_format_relation_with_company_ids(self, field_mapper):
        ...
        assert isinstance(result["relation"], list), "relation value must be a list"
        assert len(result["relation"]) == 1, (
            "relation list must contain exactly one item for single relation"
        )
        assert "id" in result["relation"][0], "relation item must have 'id' key"
