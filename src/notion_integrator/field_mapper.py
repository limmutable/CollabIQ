...
class FieldMapper:
    ...
    def map_to_notion_properties(self, extracted_data) -> Dict[str, Any]:
        ...
        collaboration_subject = f"{extracted_data.startup_name}-{extracted_data.partner_org}"
        ...
        if extracted_data.type_confidence is not None:
            properties["type_confidence"] = self._format_number(extracted_data.type_confidence)

        if extracted_data.intensity_confidence is not None:
            properties["intensity_confidence"] = self._format_number(extracted_data.intensity_confidence)

        # Metadata fields
        properties["email_id"] = self._format_rich_text(extracted_data.email_id)

        if extracted_data.classification_timestamp:
            properties["classification_timestamp"] = self._format_date(extracted_data.classification_timestamp)
