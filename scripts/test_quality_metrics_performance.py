...
def generate_mock_extraction(extraction_id: int) -> ExtractedEntities:
    ...
    entities = ExtractedEntities(
        ...
        person_in_charge=f"Person {extraction_id}" if completeness_factor >= 1 else None,
        ...
        details=f"Details for extraction {extraction_id}" if completeness_factor >= 4 else None,
        date=f"2025-11-{(extraction_id % 28) + 1:02d}" if completeness_factor >= 5 else None,
...
def main():
    ...
    try:
        ...
        console.print(f"[bold]Write Performance Test[/bold]")
        console.print(f"Recording {args.records:,} extractions for {args.providers} provider(s)...\n")
            ...
            for provider_name in provider_names:
                ...
                for i in range(args.records):
                    ...
                    tracker.record_extraction(
                        ...
                        validation_failure_reasons=["Mock validation failure"] if not validation_passed else None,
                    )
        ...
        console.print(f"Avg write time: [cyan]{sum(write_times)/len(write_times)*1000:.2f}ms[/cyan]")
        ...
        console.print(f"Throughput: [cyan]{len(write_times)/total_write_time:.1f} records/sec[/cyan]\n")

        # Read test - retrieve metrics
        console.print(f"[bold]Read Performance Test[/bold]\n")
        ...
        console.print(f"[bold green]✓ Read Test Complete[/bold green]\n")
        console.print(f"Avg read time: [cyan]{sum(read_times)/len(read_times)*1000:.2f}ms[/cyan]")
        ...
        # Performance verdict
        console.print(f"[bold]Performance Verdict[/bold]\n")
        ...
        if avg_write_ms < 10 and max_write_ms < 50:
            ...
            console.print("[yellow]⚠ FAIR[/yellow] - Write performance may need optimization")
        else:
        ...
        if args.records >= 10000:
            ...
            console.print(f"[yellow]ℹ Test with --records 10000 for production validation[/yellow]")
