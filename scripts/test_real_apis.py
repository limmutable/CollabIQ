...
def test_failover_strategy(orchestrator: LLMOrchestrator) -> None:
    ...
    try:
        ...
        logger.info(f"  Person: {entities.person_in_charge} (confidence: {entities.confidence.person:.2f})")
        logger.info(f"  Startup: {entities.startup_name} (confidence: {entities.confidence.startup:.2f})")
        logger.info(f"  Partner: {entities.partner_org} (confidence: {entities.confidence.partner:.2f})")
        logger.info(f"  Date: {entities.date} (confidence: {entities.confidence.date:.2f})")
...
def test_consensus_strategy(orchestrator: LLMOrchestrator) -> None:
    ...
    if len(available_providers) < 2:
        logger.warning(f"⚠ Consensus requires at least 2 providers, only {len(available_providers)} available")
...
def test_best_match_strategy(orchestrator: LLMOrchestrator) -> None:
    ...
    available_providers = orchestrator.get_available_providers()
    if len(available_providers) < 2:
        logger.warning(f"⚠ Best-match requires at least 2 providers, only {len(available_providers)} available")
...
def main():
    ...
    available_providers = orchestrator.get_available_providers()
    logger.info(f"✓ Orchestrator initialized with {len(available_providers)} providers: {', '.join(available_providers)}")

    if not available_providers:
        logger.error("✗ No providers available. Please add at least one API key to .env")
        return
