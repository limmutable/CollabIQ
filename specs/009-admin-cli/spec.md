...
**Created**: 2025-11-08
**Status**: Draft
**Input**: User description: "Admin CLI Enhancement - Unified collabiq command with organized subcommands for email, notion, test, errors, status, and config operations"
...
1. **Given** the admin has CollabIQ installed, **When** they run `collabiq --help`, **Then** they see all six command groups organized clearly with usage examples
...
1. **Given** configuration is loaded, **When** admin runs `collabiq config show`, **Then** they see all settings with secrets masked, source attribution (Infisical/env/default), and categories (Gmail, Notion, Gemini, LLM Providers, System)
